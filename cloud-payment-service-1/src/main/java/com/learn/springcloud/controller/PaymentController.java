...
public class PaymentController {
    ...
    public CommonsResult<Payment> forOpenFeignTimeout() {
        Payment payment = paymentService.queryPaymentById(1L);
        try {
            TimeUnit.SECONDS.sleep(3);
        } catch (InterruptedException e) {
            log.error("错误:", e.getCause());
        }
        log.info("forOpenFeignTimeout query: {}" ,serverPort);

        return new CommonsResult<>(500, "查询成功！没找到支付记录。", payment);
    }
}
